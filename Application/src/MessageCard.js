    ...
    render() {
        return (
            <li key={this.props.message.id}>
                ...
                {this.props.message.etag}{' '}
                {this.props.message.messageType}{' '}
                {new Date(Date.parse(this.props.message.createdDateTime)).toLocaleDateString()}
                {this.props.message.lastModifiedDateTime ? new Date(Date.parse(this.props.message.lastModifiedDateTime)).toLocaleDateString() : ''}
                {this.props.message.deletedDateTime ? new Date(Date.parse(this.props.message.deletedDateTime)).toLocaleDateString() : ''}
                {this.props.message.subject ? this.props.message.subject : ''}{' '}
                {this.props.message.summary ? this.props.message.summary : ''}{' '}
                {this.props.message.importance}{' '}
                {this.props.message.locale}{' '}
                {this.props.message.webUrl}{' '}
                {this.props.message.policyViolation ? this.props.message.policyViolation : ''}{' '}
                    ...
                    (() => {
                        if (this.props.message.from) {
                            if (this.props.message.from.application) {
                                return <div>
                                    {this.props.message.from.application}{' '}
                                </div>;
                            }
                            if (this.props.message.from.device) {
                                return <div>
                                    {this.props.message.from.device}{' '}
                                </div>;
                            }
                            if (this.props.message.from.conversation) {
                                return <div>
                                    {this.props.message.from.conversation}{' '}
                                </div>;
                            }
                            ...
                            if (this.props.message.attachments) {
                                {
                                    this.props.message.attachments.map(attachment => {
                                        return <AttachmentCard attachment={attachment}></AttachmentCard>
                                    })
                                }
                            }
                        })()
